...
bool LineLineIntersect(vec2 start1, //Line 1 start
	vec2 end1, //Line 1 end
	vec2 start2, //Line 2 start
	vec2 end2, //Line 2 end
	vec2 &intersect) //Output 
{
	//http://mathworld.wolfram.com/Line-LineIntersection.html

	float detL1 = det(start1, end1);
	float detL2 = det(start2, end2);
	float x1mx2 = start1.x - end1.x;
	float x3mx4 = start2.x - end2.x;
	float y1my2 = start1.y - end1.y;
	float y3my4 = start2.y - end2.y;

	float xnom = det(vmake(detL1, x1mx2), vmake(detL2, x3mx4));
	float ynom = det(vmake(detL1, y1my2), vmake(detL2, y3my4));
	float denom = det(vmake(x1mx2, y1my2), vmake(x3mx4, y3my4));
